...
import { and, asc, desc, eq, like, inArray } from "drizzle-orm";
...
const listQuery = z.object({
  ...
  // mini list ve sade alan seçimi için opsiyoneller
  ids: z.string().optional(),     // "id1,id2,id3"
  fields: z.string().optional(),  // şimdilik yoksayıyoruz
});
...
export function makeAdminController(_app: FastifyInstance) {
  return {
    ...
    update: async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const u0 = (await db.select().from(users).where(eq(users.id, id)).limit(1))[0];
      if (!u0) return reply.status(404).send({ error: { message: "not_found" } });
