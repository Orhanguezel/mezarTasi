// =============================================================
// FILE: src/modules/products/admin.reviews.controller.ts  (YENİ)
// =============================================================
...
export const adminListProductReviews: RouteHandler = async (req, reply) => {
  ...
  const q = (req.query || {}) as { only_active?: string | number | boolean; order?: "asc" | "desc" };
  const conds = [eq(productReviews.product_id, id)];
  if (q.only_active !== undefined) {
    const v = String(q.only_active).toLowerCase();
    conds.push(eq(productReviews.is_active, (v === "1" || v === "true") as any));
  }
  const orderBy = q.order === "asc" ? asc(productReviews.review_date) : desc(productReviews.review_date);
  const rows = await db.select().from(productReviews).where(and(...conds)).orderBy(orderBy);
...
export const adminCreateProductReview: RouteHandler = async (req, reply) => {
  ...
  try {
    ...
    const input = productReviewCreateSchema.parse({ ...(req.body || {}), product_id: id });
    const row: any = {
      ...input,
...
export const adminUpdateProductReview: RouteHandler = async (req, reply) => {
  ...
  try {
    const patch = productReviewUpdateSchema.parse({ ...(req.body || {}), product_id: id });
    const upd: any = { ...patch, updated_at: now() };
    if (patch.review_date !== undefined) {
      upd.review_date = patch.review_date ? new Date(patch.review_date) : null;
    }
    await db
      .update(productReviews)
      .set(upd)
      .where(and(eq(productReviews.id, reviewId), eq(productReviews.product_id, id)));
    const [row] = await db.select().from(productReviews).where(eq(productReviews.id, reviewId)).limit(1);
...
export const adminDeleteProductReview: RouteHandler = async (req, reply) => {
  const { id, reviewId } = req.params as { id: string; reviewId: string };
  await db.delete(productReviews).where(and(eq(productReviews.id, reviewId), eq(productReviews.product_id, id)));
  return reply.send({ ok: true });
};
