...
export function ModelsPage({ onNavigate, onProductDetail }: ModelsPageProps) {
  ...
  // --- normalize helper (diakritik + TR char'ları sadeleştir) ---
  ...
  // ✅ name/slug → iç anahtar map'i (bastaslari|modeller|mermer|granit|sutunlu)
  ...
  useEffect(() => {
    const loadAllModels = () => {
      ...
      const convertedDynamicProducts: TombstoneModel[] = dynamicProducts
        ...
        .map((product) => ({
          id: product.id + 1000, // çakışmayı engelle
    ...
    // Kategori map'i değiştiğinde yeniden dönüştür
  ...
  // Statik showcase (featured bölümünde kullanılıyor)
  const models: TombstoneModel[] = [
    ...
    // ... diğer modeller aynı
    {
      ...
      image: tombstoneImage1,
      description:
        "Müşteri isteklerine göre özel olarak tasarlanan granit mezar baş taşı modeli",
      featured: true,
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Süreç, Öne çıkanlar, CTA ve Modal kısımları aynı */}
      {/* ... */}
