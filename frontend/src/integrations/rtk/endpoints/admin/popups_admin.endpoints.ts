...
// "" -> null olacak şekilde düzelt
const strOrNull = (v: unknown): string | null => {
  if (v == null) return null;
  if (typeof v === "string") {
    const s = v.trim();
    return s.length ? s : null; // boş string -> null
  }
...
// unknown → PopupAdminView (undefined yazmıyoruz)
const toView = (x: unknown): PopupAdminView => {
  ...
  const base: PopupAdminView = {
    ...
    duration_seconds: Number.isFinite(Number(r["duration_seconds"])) ? Number(r["duration_seconds"]) : null,
  };

  if (createdAt !== undefined) (base as any).created_at = createdAt;
  if (updatedAt !== undefined) (base as any).updated_at = updatedAt;
