// =============================================================
// FILE: src/integrations/metahub/realtime/channel.ts
// =============================================================
...
type Listener = {
  event: string;
  // Not: exactOptionalPropertyTypes altında 'filter?: T' demek "property yok" olmalı.
  // Eğer 'undefined' değerini de kabul ettirmek istersen: `filter?: Record<string, unknown> | undefined`
...
class Channel {
  ...
  on<T = unknown>(event: string, a: Handler<T> | Record<string, unknown>, b?: Handler<T>): this {
    ...
    const cbFn: Handler<T> = hasFilter ? (b as Handler<T>) : (a as Handler<T>);
    const filterObj = hasFilter ? (a as Record<string, unknown>) : undefined;

    // listener'ı tekilleştir
    if (!this.listeners.some(l => l.event === event && l.cb === (cbFn as Handler<unknown>) && shallowEqual(l.filter, filterObj))) {
      // ✅ filter tanımlıysa property'yi ekle, değilse hiç yazma
      const toPush: Listener = (filterObj
        ? { event, filter: filterObj, cb: cbFn as Handler<unknown> }
        : { event, cb: cbFn as Handler<unknown> }) as Listener;

      this.listeners.push(toPush);
    }
    return this;
  }
